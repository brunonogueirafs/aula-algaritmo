...
function listProducts() {
  for (let x = 0; x <= products.length - 1; x++) {
    ...
    seeButton.setAttribute("data-bs-target","#modalVer");
    seeButton.onclick = () => verProduto(x);
...
function verProduto(index) {
  produtoSendoVisitado = products[index];

  console.log(produtoSendoVisitado);

  const modalBody = document.querySelector("#modalVer div.modal-body");
  modalBody.innerHTML = "";

  console.log(modalBody);

  const productIdDiv = document.createElement("div");
  productIdDiv.innerText = `Id: ${produtoSendoVisitado.product_id}`;

  const productNameDiv = document.createElement("div");
  productNameDiv.innerText = `Name: ${produtoSendoVisitado.product_name}`;

  const productStockDiv = document.createElement("div");
  productStockDiv.innerText = `Stock: ${produtoSendoVisitado.product_stock}`;

  const productPriceCostDiv = document.createElement("div");
  productPriceCostDiv.innerText = `Pricecost: ${produtoSendoVisitado.product_price_cost}`;

  const productPriceDiv = document.createElement("div");
  productPriceDiv.innerText = `Price: ${produtoSendoVisitado.product_price}`;




  modalBody.appendChild(productIdDiv);

  modalBody.appendChild(productNameDiv);

  modalBody.appendChild(productStockDiv);

  modalBody.appendChild(productPriceCostDiv);

  modalBody.appendChild(productPriceDiv);
...
function confirmarDeletarProduto(index) {
  ...
  document.getElementById("deleteContent").innerText = 
  `${products[index].product_id}
   ${products[index].product_name} 
   ${products[index].product_stock} 
   ${products[index].product_price_cost}
